...
void simple_reqwt_compare ()
{
	...
	if (read_dword_fcs(finish, true, false) > N_CPU || read_dword_fcs(finish, true, false) == 0) {
		...
		for (uint64_t i = 0; i < BUF_LENGTH; i++) {
			...
			write_dword_fcs(buffer1+i, buffer1_val + buffer2_val, true, false);
		}
	}
	...
	if (hartid == 0) {
		...
		printf ("%0d: Time 1 = %llu\n", hartid, checkpoint[1] - checkpoint[0]);
		printf ("%0d: Time 2 = %llu\n", hartid, checkpoint[2] - checkpoint[1]);
	}
