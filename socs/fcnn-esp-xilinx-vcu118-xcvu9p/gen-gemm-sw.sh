...
do
    ...
    cp soft-build/ariane/sysroot/applications/test/gemm_stratus.exe test/gemm-os/gemm-os-${length}.exe

    # MESI
    GEMM_LEN=$length ENABLE_SM=1 IS_ESP=1 COH_MODE=0 make gemm_stratus-app-clean gemm_stratus-app
    cp soft-build/ariane/sysroot/applications/test/gemm_stratus.exe test/gemm-mesi/gemm-mesi-${length}.exe

    # DMA
    GEMM_LEN=$length ENABLE_SM=1 IS_ESP=1 COH_MODE=1 make gemm_stratus-app-clean gemm_stratus-app
    cp soft-build/ariane/sysroot/applications/test/gemm_stratus.exe test/gemm-dma/gemm-dma-${length}.exe
