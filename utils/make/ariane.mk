...
$(SOFT_BUILD)/prom.exe: $(SOFT_BUILD)/startup.o $(SOFT_BUILD)/uart.o $(SOFT_BUILD)/main.o $(BOOTROM_PATH)/linker.lds
	...
	$(QUIET_CC) $(CROSS_COMPILE_ELF)gcc \
		...
		$(TEST_SUITE)/00_systest_helper.c \
		-o $@
...
$(SOFT_BUILD)/systest.exe: $(TEST_SUITE)/00_systest*.c $(TEST_SUITE)/$(TEST_ID)_systest_*.c $(SOFT_BUILD)/uart.o
	@echo "Compiling test $(TEST_ID)"
	@mkdir -p $(SOFT_BUILD)
	$(QUIET_CC) $(CROSS_COMPILE_ELF)gcc $(RISCV_CFLAGS) \
	$(TEST_SUITE)/00_systest_*.c  \
	$(TEST_SUITE)/$(TEST_ID)_systest_*.c  \
