...
void mesh_gen::load_input()
{
    ...
    // Load
    {
        ...
        for (uint16_t b = 0; b < 1; b++)
        {
            ...
            uint32_t length = num_hash_table;
#else
...
void mesh_gen::store_output()
{
    ...
    // Store
    {
        ...
        bool ping = true;
#if (DMA_WORD_PER_BEAT == 0)
        uint32_t store_offset = (num_hash_table) * 1;
#else
        uint32_t store_offset = round_up(num_hash_table, DMA_WORD_PER_BEAT) * 1;
#endif
...
void mesh_gen::compute_kernel()
{
    ...
    bool ping = true;
    {
        for (uint16_t b = 0; b < 1; b++)
        {
            uint32_t in_length = num_hash_table;
            uint32_t out_length = num_hash_table;
