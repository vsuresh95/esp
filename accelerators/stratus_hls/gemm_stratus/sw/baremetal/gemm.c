...
#define SLD_GEMM 0x05a
#define DEV_NAME "sld,gemm_stratus"

/* <<--params-->> */
const int32_t gemm_m = 16;
const int32_t gemm_n = 16;
const int32_t gemm_k = 16;
...
int main(int argc, char * argv[])
{
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, GEMM_GEMM_M_REG, gemm_m);
		iowrite32(dev, GEMM_GEMM_N_REG, gemm_n);
		iowrite32(dev, GEMM_GEMM_K_REG, gemm_k);
		iowrite32(dev, GEMM_GEMM_BATCH_REG, gemm_batch);
