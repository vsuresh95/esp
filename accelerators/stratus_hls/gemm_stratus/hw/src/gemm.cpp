		...
		switch(load_state){
			...
			case LOAD_CONFIG:{
				...
				// 	HLS_DEFINE_PROTOCOL("load-config-sig");
					...
					matrix_chk_in_sig.write(1);
					matrix_rem_in1_sig.write(0);
					matrix_rem_in2_sig.write(0);
					matrix_chk_out_sig.write(1);
					matrix_rem_out_sig.write(0);
					load_cfg_sig.write(LESS_THAN_ROW);
					loadable_rows_sig.write(matrix_d2);
					loadable_chunk_sig.write(matrix_d2);
					index_d1_incr_sig.write(matrix_d2);
...
void gemm::store_output()
{
	...
	while(true)
	{
		...
		switch(store_state){
			...
			case STORE_DATA_REQ:{
				int32_t length = matrix_d1*matrix_d3;
	...
	while(true){
		...
		// for (uint24_t a = 0; a < ninputs; a++)
		{
			...
			for (uint24_t d1 = 0; d1 < matrix_d1; d1 += loadable_rows)
			{
				...
				{
					plm_offset_m1 = 0; //d1i * matrix_d2;

					for (uint16_t d2i = 0; d2i < loaded_rows_d3; d2i++)
					{
					plm_offset_m2 = 0; //d2i * matrix_d2;
