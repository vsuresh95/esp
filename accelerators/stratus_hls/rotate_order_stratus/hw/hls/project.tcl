...
#
define_hls_module rotate_order ../src/rotate_order.cpp
...
foreach dma [list 64] {
	...
	define_sim_config "BEHAV_FX$fx\_DMA$dma" "rotate_order BEH" "tb TESTBENCH_FX$fx\_DMA$dma" -io_config IOCFG_FX$fx\_DMA$dma -argv $DEFAULT_ARGV

	foreach cfg [list BASIC] {
	    set cname $cfg\_FX$fx\_DMA$dma
	    define_hls_config rotate_order $cname -io_config IOCFG_FX$fx\_DMA$dma --clock_period=$CLOCK_PERIOD $COMMON_HLS_FLAGS -DHLS_DIRECTIVES_$cfg
	    if {$TECH_IS_XILINX == 1} {
		define_sim_config "$cname\_V" "rotate_order RTL_V $cname" "tb TESTBENCH_FX$fx\_DMA$dma" -io_config IOCFG_FX$fx\_DMA$dma -argv $DEFAULT_ARGV -verilog_top_modules glbl
	    } else {
		define_sim_config "$cname\_V" "rotate_order RTL_V $cname" "tb TESTBENCH_FX$fx\_DMA$dma" -io_config IOCFG_FX$fx\_DMA$dma -argv $DEFAULT_ARGV
	    }
	}
    }
}
