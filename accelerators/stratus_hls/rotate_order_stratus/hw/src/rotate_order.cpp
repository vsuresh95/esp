...
void rotate_order::load_input()
{
    ...
    // Load
    {
        ...
        for (uint16_t b = 0; b < 1; b++)
        {
            ...
            uint32_t length = 16 * block_size;
#else
            uint32_t length = round_up(16 * block_size, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void rotate_order::store_output()
{
    ...
    // Store
    {
        ...
        bool ping = true;
#if (DMA_WORD_PER_BEAT == 0)
        uint32_t store_offset = (16 * block_size) * 1;
#else
        uint32_t store_offset = round_up(16 * block_size, DMA_WORD_PER_BEAT) * 1;
#endif
...
void rotate_order::compute_kernel()
{
    ...
    bool ping = true;
    {
        for (uint16_t b = 0; b < 1; b++)
        {
            uint32_t in_length = 16 * block_size;
            uint32_t out_length = 16 * block_size;
