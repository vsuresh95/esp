...
#define SLD_ROTATE_ORDER 0x059
#define DEV_NAME "sld,rotate_order_stratus"

/* <<--params-->> */
const int32_t num_channel = 16;
...
/* <<--regs-->> */
#define ROTATE_ORDER_LOGN_SAMPLES_REG 0x40
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, VENDOR_SLD, SLD_ROTATE_ORDER, DEV_NAME);
	if (ndev == 0) {
		printf("rotate_order not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
			iowrite32(dev, ROTATE_ORDER_LOGN_SAMPLES_REG, logn_samples);
			...
			unsigned local_cons_rdy_flag_offset = READY_FLAG_OFFSET;
			unsigned local_cons_vld_flag_offset = VALID_FLAG_OFFSET;
			unsigned local_prod_rdy_flag_offset = acc_offset + READY_FLAG_OFFSET;
			unsigned local_prod_vld_flag_offset = acc_offset + VALID_FLAG_OFFSET;
			...
			for (j = 0; j < num_channel; j++)
			{
				...
				// Read back output
				validate_buf(&mem[acc_offset], (gold + out_len));
