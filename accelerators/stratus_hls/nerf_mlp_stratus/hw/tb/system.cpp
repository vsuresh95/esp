...
void system_t::load_memory()
{
    ...
    // weights
    for (int i = 0; i < 1; i++)
        for (int j = 0; j < LAYER_0_INPUTS*LAYER_0_OUTPUTS; j++)
            in[i * in_words_adj + j] = (int64_t) (1);

    // biases
    for (int i = 0; i < 1; i++)
        for (int j = LAYER_0_INPUTS*LAYER_0_OUTPUTS; j < weights_offset; j++)
            in[i * in_words_adj + j] = (int64_t) (2);

    // inputs
    for (int i = 0; i < 1; i++)
        for (int j = weights_offset; j < in_size; j++)
            in[i * in_words_adj + j] = (int64_t) (3);
...
int system_t::validate()
{
    ...
    for (int i = 0; i < out_size; i++)
        ...
        {
            errors++;
            ESP_REPORT_INFO("ping[%d] = %lu out[%d] = %lu\n", i, ping[i], i, out[i]);
        }
