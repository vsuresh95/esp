...
void nerf_mlp::load_input()
{
    ...
    // Load
    {
        ...
        for (uint16_t b = 0; b < 1; b++)
        {
            ...
            uint32_t length = 256*256*batch_size;
#else
            uint32_t length = round_up(256*256*batch_size, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void nerf_mlp::store_output()
{
    ...
    // Store
    {
        ...
        bool ping = true;
#if (DMA_WORD_PER_BEAT == 0)
        uint32_t store_offset = (256*256*batch_size) * 1;
#else
        uint32_t store_offset = round_up(256*256*batch_size, DMA_WORD_PER_BEAT) * 1;
#endif
        ...
        for (uint16_t b = 0; b < 1; b++)
        {
            ...
            uint32_t length = 1;
#else
            uint32_t length = round_up(1, DMA_WORD_PER_BEAT);
#endif
            // Chunking
...
void nerf_mlp::compute_kernel()
{
    ...
    bool ping = true;
    {
        for (uint16_t b = 0; b < 1; b++)
        {
            uint32_t in_length = 256*256*batch_size;
