...
#include "gemm_3_block.hpp"
#include "gemm_3_block_directives.hpp"

// Functions

#include "gemm_3_block_functions.hpp"

// Processes

void gemm_3_block::load_input()
{
...
void gemm_3_block::store_output()
{
    // Reset
    {
...
void gemm_3_block::compute_kernel()
{
    // Reset
    {
    ...
    bool ping_n = true;
    {
        ...
        {   
            ...
            for (int32_t rem_n = length_n; rem_n > 0; rem_n -= PLM_IN_WORD/2)
            {
                ...
                uint32_t acc_region = ((PLM_IN_WORD/2)/gemm_n)/NUM_GEMM;

                // Computing phase implementation
                for (uint32_t acc_inst = 0; acc_inst < NUM_GEMM; acc_inst++)
                {
                    HLS_UNROLL_LOOP(ON, "num_gemm");
                    for (uint32_t m = acc_inst * acc_region; m < (acc_inst + 1) * acc_region; m++)
                    {
