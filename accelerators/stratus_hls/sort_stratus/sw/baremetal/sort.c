...
static int validate_sorted(float *array, float* gold, int len)
{
	...
	for (i = 0; i < len; i += 2, src += 8, dst += 8) {
		...
		if (out_data.value_32_1 != gold_data.value_32_1) {
			rtn++;
		}
		if (out_data.value_32_2 != gold_data.value_32_2) {
			rtn++;
		}
