...
#define SLD_GEMM_4_2D_BLOCK 0x074
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_GEMM_4_2D_BLOCK, DEV_NAME);
	if (ndev == 0) {
		printf("gemm_4_2d_block not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, GEMM_4_2D_BLOCK_GEMM_M_REG, gemm_m);
		iowrite32(dev, GEMM_4_2D_BLOCK_GEMM_N_REG, gemm_n);
		iowrite32(dev, GEMM_4_2D_BLOCK_GEMM_K_REG, gemm_k);
