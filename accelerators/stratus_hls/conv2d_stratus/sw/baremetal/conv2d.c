...
static void sw_run(native_t *in, native_t *weights, native_t *bias, native_t *out)
{
    int32_t pad_dim = 0;
...
static int validate_buf(token_t *out, native_t *gold)
{
	...
	for (j = 0; j < out_len; j++) {
		...
		if (gold[j] == 0) {
			...
			native_t rel_err = (gold[j] - val) / gold[j];
...
static void init_buf (token_t *in, native_t * gold)
{
	...
	sw_run(gold, &gold[weights_offset], &gold[bias_offset], &gold[out_offset]);
}
