...
{
    if (effective_rows < filter_dim)
        return 0;

    return (((uint16_t) (effective_rows - filter_dim)) >> stride_log2) + 1;
}
    ...
    uint16_t *loadable_chan_sz, uint16_t *chan_rem_sz)
{
    ...
    if (is_padded) {
        ...
        *output_w = (((uint16_t) (width - 1)) >> stride_log2) + 1;
        pad_h = max((int32_t) 0,
                (int32_t) (((output_h - 1) * stride) + filter_dim - height));
        pad_w = max((int32_t) 0,
                (int32_t) (((*output_w - 1) * stride) + filter_dim - width));
        *pad_top = pad_h >> 1;
        *pad_bottom = pad_h - *pad_top;
        *pad_left = pad_w >> 1;
    } else {
        output_h = ((((uint16_t)(height - filter_dim)) >> stride_log2)) + 1;
        *pad_top = 0;
        *pad_bottom = 0;
        *pad_left = 0;
        *output_w = (((uint16_t) (width - filter_dim)) >> stride_log2) + 1;
    }
    ...
    *max_cacheable_size_init = *max_cacheable_rows_init * width;
    ...
    uint16_t output_rows_per_chunk = conv2d_chunk_output_rows(
	    *max_cacheable_rows, filter_dim, stride_log2);
    uint16_t output_rows_per_chunk_init = conv2d_chunk_output_rows(
	    init_effective_rows, filter_dim, stride_log2);
    uint16_t remaining_output_rows;

    *feature_row_incr = output_rows_per_chunk * stride;
    *feature_row_incr_init = output_rows_per_chunk_init * stride - *pad_top;
    if (output_rows_per_chunk_init >= output_h) {
	    *total_input_chunks = 1;
    } else {
	    remaining_output_rows = output_h - output_rows_per_chunk_init - 1;
	    *total_input_chunks = (remaining_output_rows / output_rows_per_chunk) + 2;
    }
