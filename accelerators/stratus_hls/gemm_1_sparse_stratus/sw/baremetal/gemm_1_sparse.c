...
#define SLD_GEMM_1_SPARSE 0x071
#define DEV_NAME "sld,gemm_1_sparse_stratus"

/* <<--params-->> */
const int32_t h_order_size = 64;
const int32_t gemm_m = 64;
const int32_t gemm_n = 64;
const int32_t gemm_k = 64;
const int32_t gemm_batch = 1;
const int32_t var_size = 128;
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_GEMM_1_SPARSE, DEV_NAME);
	if (ndev == 0) {
		printf("gemm_1_sparse not found\n");
		return 0;
	}
	...
	for (n = 0; n < ndev; n++) {
...
#else
		{
			...
			/* <<--regs-config-->> */
		iowrite32(dev, GEMM_1_SPARSE_H_ORDER_SIZE_REG, h_order_size);
		iowrite32(dev, GEMM_1_SPARSE_GEMM_M_REG, gemm_m);
		iowrite32(dev, GEMM_1_SPARSE_GEMM_N_REG, gemm_n);
		iowrite32(dev, GEMM_1_SPARSE_GEMM_K_REG, gemm_k);
		iowrite32(dev, GEMM_1_SPARSE_GEMM_BATCH_REG, gemm_batch);
		iowrite32(dev, GEMM_1_SPARSE_VAR_SIZE_REG, var_size);
